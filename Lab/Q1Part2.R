...
head(df)  
p=ggplot(df)+geom_col(aes(x=Age_Groups,y=Number_of_Loans),size=1)+geom_line(aes(x=Age_Groups,y=2500*Bad_Rate),size=1.5,group = 1,color='yellow', position = "identity",  stat = "identity")
p
